...
import { ENV_HASH_ROUND_KEY, ENV_JWT_ACCESS_SECRET_KEY, ENV_JWT_REFRESH_SECRET_KEY } from '../common/const/env-keys-values.const';
...
export class AuthService {
    ...
    async authenticateWithEmailAndPassword(user: Pick<UsersEntity, 'email' | 'password'>) {
        ...
        if (!existingUser) throw new BadRequestException('이메일 또는 비밀번호가 틀렸습니다.');

        const passOk = await bcrypt.compare(user.password, existingUser.password);

        if (!passOk) throw new BadRequestException('이메일 또는 비밀번호가 틀렸습니다..');
