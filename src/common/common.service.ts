...
export class CommonService {
    ...
    private async cursorPaginte<T extends CoreEntity>(
        ...
        const lastItem = result.length > 0 && result.length === take ? result[result.length - 1] : null;
    ...
    private parseWhereFilters<T extends CoreEntity>(dto: CorePaginationDto)
        : FindOptionsWhere<T> {
    ...
    private parseOrderFilters<T extends CoreEntity>(dto: CorePaginationDto)
        : FindOptionsOrder<T> {
    ...
    private parseWhereFilter<T extends CoreEntity>(key: string, value: unknown)
        : FindOptionsWhere<T> {
        ...
        if (!filterMapper) {
            throw new BadRequestException(`${operator}(은)는 아직 지원하지 않습니다.`)
        }
    ...
    private parseOrderFilter<T extends CoreEntity>(key: string, value: unknown)
        : FindOptionsOrder<T> {
